defmodule MySensors.Types do
  ...
  def presentation do
    [
      ...
      {S_DIMMER, 4,
       "Dimmable device of some kind	V_STATUS (on/off), V_PERCENTAGE (dimmer level 0-100), V_WATT"},
      ...
      {S_POWER, 13,
       "Power measuring device, like power meters	V_WATT, V_KWH, V_VAR, V_VA, V_POWER_FACTOR"},
      ...
      {S_LIGHT_LEVEL, 16,
       "Light sensor	V_LIGHT_LEVEL (uncalibrated percentage), V_LEVEL (light level in lux)"},
      ...
      {S_HVAC, 29,
       "Thermostat/HVAC device	V_STATUS, V_TEMP, V_HVAC_SETPOINT_HEAT, V_HVAC_SETPOINT_COOL, V_HVAC_FLOW_STATE, V_HVAC_FLOW_MODE, V_HVAC_SPEED"},
  ...
  def variable do
    [
      ...
      {V_STATUS, 2,
       "Binary status. 0=off 1=on	S_BINARY, S_DIMMER, S_SPRINKLER, S_HVAC, S_HEATER, S_WATER_QUALITY"},
      {V_PERCENTAGE, 3, "Percentage value. 0-100 (%)	S_DIMMER, S_COVER"},
      {V_PRESSURE, 4, "Atmospheric Pressure	S_BARO"},
      {V_FORECAST, 5,
       "Whether forecast. One of 'stable', 'sunny', 'cloudy', 'unstable', 'thunderstorm' or 'unknown'	S_BARO"},
      ...
      {V_ARMED, 15,
       "Armed status of a security sensor. 1=Armed, 0=Bypassed	S_DOOR, S_MOTION, S_SMOKE, S_SPRINKLER, S_WATER_LEAK, S_SOUND, S_VIBRATION, S_MOISTURE"},
      {V_TRIPPED, 16,
       "Tripped status of a security sensor. 1=Tripped, 0=Untripped	S_DOOR, S_MOTION, S_SMOKE, S_SPRINKLER, S_WATER_LEAK, S_SOUND, S_VIBRATION, S_MOISTURE"},
      {V_WATT, 17,
       "Watt value for power meters	S_POWER, S_BINARY, S_DIMMER, S_RGB_LIGHT, S_RGBW_LIGHT"},
      ...
      {V_HVAC_FLOW_STATE, 21,
       "Mode of header. One of 'Off', 'HeatOn', 'CoolOn', or 'AutoChangeOver'	S_HVAC, S_HEATER"},
      {V_HVAC_SPEED, 22,
       "HVAC/Heater fan speed ('Min', 'Normal', 'Max', 'Auto')	S_HVAC, S_HEATER"},
      {V_LIGHT_LEVEL, 23,
       "Uncalibrated light level. 0-100%. Use V_LEVEL for light level in lux.	S_LIGHT_LEVEL"},
      ...
      {V_CUSTOM, 48,
       "Custom messages used for controller/inter node specific commands, preferably using S_CUSTOM device type.	S_CUSTOM"},
      {V_POSITION, 49,
       "GPS position and altitude. Payload: latitude;longitude;altitude(m). E.g. '55.722526;13.017972;18'	S_GPS"},
      ...
      {V_POWER_FACTOR, 56,
       "Ratio of real power to apparent power: floating point value in the range [-1,..,1]	S_POWER"}
    ]
  end
