defmodule MySensors.Gateway do
  ...
  Send a message to the MySensors network, waiting for an ack from the destination
  """
  @spec sync_message(Types.id, Types.id, Types.command, Types.type, String.t, timeout) :: :ok | :timeout
  def sync_message(node_id, child_sensor_id, command, type, payload \\ "", timeout \\ @ack_timeout) do
  ...
  def handle_call({:version}, _from = {pid, _ref}, state) do
    {:reply, _send_message(0, 255, :internal, I_VERSION), %{state | version_handlers: state.version_handlers ++ [pid]}}
  end
