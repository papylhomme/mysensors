defmodule MySensors.Message do
  ...
  @type sensor_updated :: %__MODULE__{
          node_id: Types.id(),
          child_sensor_id: Types.id(),
          command: :set,
          ack: boolean,
  ...
  def new(node_id, child_sensor_id, command, type, payload \\ "", ack \\ false) do
    %__MODULE__{
      node_id: node_id,
  ...
  def parse(input) do
    try do
      [node_id, child_sensor_id, command, ack, type, payload] =
        String.split(input, ~r/[^a-zA-Z0-9-_]/, parts: 6)
  ...
  def serialize(msg) do
    ...
    "#{msg.node_id};#{msg.child_sensor_id};#{cmd};#{if msg.ack, do: 1, else: 0};#{t};#{
      msg.payload
    }"
  end
end
